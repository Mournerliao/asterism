...
`apps/web` is the primary surface (响应式 Web，MVP 优先端)，覆盖登录、同步、浏览（卡片/列表 + 虚拟滚动）、多维筛选、关键词搜索、标签、集合、笔记、统计仪表盘与导入导出；Phase 2 已交付 AI 整理建议（BYOK）和批量整理，ADR 0029 进一步把 AI 整理调整为目标优先的持久任务。success = 用户能在几秒内从上千个 star 里找到仓库，也能表达一个完整整理目标，由系统承担全库候选发现与技术分批，只审批真正有风险的动作。
...
6. **委托目标，而非管理批次**：AI 整理从用户想得到的秩序开始；选择范围只是可选上下文，内部页数和调用上限不能转化为用户的整理劳动。
