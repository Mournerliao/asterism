...
AI 分类固定输入为已持久化的公开仓库内容（owner/name、描述、语言、GitHub topics）以及当前用户现有标签 / 集合；当前用户私有笔记只有在用户明确启用 `include_notes_in_ai` 后才加入 Generation 输入。首次分类前必须向用户展示实际字段与目标 Provider；关闭笔记发送后不得把笔记正文传给任何 Adapter。不得读取其他用户的笔记或组织数据，也不得把 README 发送给 Generation Provider。
