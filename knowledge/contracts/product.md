...
- **集合盘（Collection Dial）**：ADR 0033 已接受 Browse 内的临时底部集合盘作为直接整理入口。用户拿起单个仓库，或在选择模式中拿起已选仓库后，集合盘才出现；当前集合沿弧形轨道居中，文件夹的闭合 / 打开状态表达可投放容器。点击文件夹或使用 Q/E 只选择目标，Enter 或明确按钮确认写入；直接拖放到文件夹并在盘内松手可一步确认，盘外松手或 Escape 取消。候选在拿起时形成冻结快照，embedding 不可用时静默降级为最近使用与稳定顺序；推荐只改变已有集合的次序，不自动修改 canonical，也不恢复 Generation、Provider 或 BYOK。写入等待 Postgres 权威结果后才宣告成功，失败保留仓库范围与目标以供重试或取消；已有关系幂等处理，多选沿用持久批量操作语义，每次成功提供独立短期 Undo。「更多集合」与「新建集合」必须承接并冻结当前范围，创建成功后自动加入。首个生产规格只覆盖 Browse。throwaway prototype 已退役，`?prototype=collection-dial` 回到正常 Browse。GitHub #34 已由维护者关闭；验收中发现的问题另开 ticket。
