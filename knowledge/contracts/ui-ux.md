...
Phase 2 的 AI 自动分类结果是整理建议草稿，不是已持久化状态。AI 入口只接受用户当前手动选择或“全选当前筛选结果”的仓库，调用前显示准确数量；不得提供后台扫描或自动整理整个库的入口。审阅界面必须明确区分“添加关系”“移除关系”和“建议新建分类”，移除项不得用含糊的未选中状态表达；默认优先展示并复用现有分类，新分类需经过名称规范化与大小写、空白、近似名称检查，且必须由用户单独勾选确认。用户可逐仓库、逐建议取消，确认动作显示最终将创建的分类以及将添加、移除的关系数量；提交后复用批量整理的部分失败结果与重试模式。
