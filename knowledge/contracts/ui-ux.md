...
### Collection Dial Pattern · 集合盘模式

Collection Dial 是 Browse 中由用户直接整理动作临时唤起的底部操作层，不是新页面、常驻导航或自动整理面板。ADR 0033 接受的体验骨架已由生产组件按共享边界落地；许可文件夹矢量位于 `packages/ui/src/assets/collection-dial/`，运行时 FolderArtwork 使用设计 token 着色。

- 空间模型：集合以文件夹容器沿弧形轨道排布，当前目标转入中央并完整显色；相邻项按距离平移、旋转、缩放和降权，标签始终保持水平可读。半圆感来自对象排布，背景只使用从页面内容自然过渡到半透明模糊的操作层，不绘制实体 basin、rim 或大面积装饰光晕。
- 容器状态：闭合文件夹表达可选集合，当前拖放命中或写入中的目标以打开状态表达接收。正式资源沿用经许可来源提取的透明矢量文件夹和 Asterism 浅蓝色阶；不得改回 3D、重拟物、自绘替代图标或额外椭圆落地阴影。
...
- 响应式：可见窗口根据集合盘容器宽度采用奇数项，当前目标保持居中；已验证方向为宽屏最多 7 项、窄屏最多 3 项。移动端以常驻 Grip 作为拖动起点并保留点按等价路径，不使用整卡长按；生产规格须继续验证列表滚动与拖动不会互相误触。
