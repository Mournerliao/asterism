...
AI 整理采用 ADR 0029 的目标优先任务模型。默认入口允许用户描述结果；手动选择或当前筛选结果只作为可选范围约束，不得继续作为 AI 开始工作的门槛。同步完成后可以提示少量整理机会，但提示本身不得调用 BYOK 或阻断当前任务；首次同步与后续增量同步必须使用不同语境，不得要求用户清空强制队列。

开始整理任务前，界面必须解释系统对目标的理解、完整候选数量与候选依据，并披露 Provider、发送字段、截断边界及费用 / 工作量预估。内部分页、重试和跨页归并表现为一个可暂停、可恢复的整体任务，禁止要求用户自行决定下一批 50 条。

审阅以动作组为主、仓库明细为可展开的抽查与排除路径。加入已有分类可聚合确认；新分类必须单独确认名称、代表成员及与已有分类的近似关系；移除已有 canonical 关系独立呈现且不能默认混入普通添加；不确定项明确保持不操作。确认动作始终显示最终新建分类数、添加关系数与移除关系数，成功后切换到既有持久化批量操作进度与失败恢复。

每个已执行的 AI 整理任务提供明确的任务级撤销入口与结果反馈。撤销只恢复该任务实际改变且之后未发生有效用户变更的标签 / 集合关系；冲突项保守跳过并说明原因，不得删除后来被用户继续使用的分类。撤销自身也必须可恢复并准确报告跳过与部分失败。现有 selection-first 草稿 UI 在替代流程交付前属于兼容路径，不得被当作调整后产品承诺已经完成。完整 Phase 2.1 规格见 GitHub #23。
