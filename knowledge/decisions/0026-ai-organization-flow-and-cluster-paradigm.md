...
- Status: Accepted（§1–§6 及 §7 的隐形混合搜索继续有效；§8 已被 ADR 0027 取代，§7 的语义星图已被 ADR 0028 取代，“系统不主动提示”的产品性格已被 ADR 0029 的可忽略整理机会局部取代）
- Date: 2026-07-23
...
真实 518 条个人 Star 验证表明 §8 假设不成立：全库无法形成稳定、互斥、可命名的主题分区。ADR 0027 已用用户确认的**局部语义邻域**取代涌现簇与 promotion。随后继续按用户任务审视 §7，确认二维语义星图没有混合搜索与局部邻域之外的独占职责，且裸点云增加判断成本、投影误读与位置漂移；ADR 0028 已移除正式星图。本 ADR 的检索优先、双平面、浏览器内 embedding 与隐形混合搜索继续有效。
