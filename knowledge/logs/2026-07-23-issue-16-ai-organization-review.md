...
- 1440×900，light / dark：页脚固定，长内容只在主体滚动；双轴复审后壳层宽度收敛到
  合同规定的 448px。
...
双轴复审后将审阅 Dialog 恢复为合同规定的 `28rem` 默认壳层，并把持久化审阅模型集中到
`packages/core`。同时将 pending suggestion ID 与 mutation 的真实 pending 状态绑定，避免
settled 后遗留的 variables 持续锁定审阅控件。
