...
- 回填先经 `packages/db` 求 owner 的缺失 / 模型失配 / 内容失配集合。全量新鲜时不创建 Worker；非空才准备运行时。每条向量经普通用户会话、带 `userId` 的 owner-only RLS upsert 写入，成功一条即形成续跑检查点。
...
最终门禁：`pnpm lint`、`pnpm typecheck`、`pnpm test`、`pnpm build` 全绿。测试计数为 core 155、db 60、Supabase Functions 94、web 164；生产构建仅保留既有主 chunk warning。
