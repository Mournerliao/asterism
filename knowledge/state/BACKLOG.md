...
- [ ] **检索优先范式继续落地（ADR 0026 Accepted，2026-07-23）**：#18 `user_repo_embeddings` + owner-only RLS、#19 浏览器内 q8 embedding 运行时 / 同源资产 / 增量回填 / 降级 UX、#20 隐形混合搜索与 #21 石墨语义星图（确定性 PCA + 分层 Canvas2D 渲染 + 点亮路径）均已完成。剩余为 **#22 密度聚类 + promotion**；#22 依赖 #21（已完成），可立即领取。见 ADR 0026、各记录日志。
