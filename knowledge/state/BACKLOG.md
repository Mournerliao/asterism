...
- [x] **Phase 2 批量整理实现（GitHub #11）**（2026-07-19）：Browse 支持手动选择与“全选当前筛选结果”快照；`bulk-organize` 以受信、有界批次持久化并执行逐关系 tag / collection 变更，保留成功项、区分可重试/终止失败并支持刷新恢复与明确结束。添加已有关系、移除缺失关系均为幂等成功，普通客户端仅可读取本人账本。GitHub #12 继续交付所选仓库 JSON / CSV / Markdown 导出。
