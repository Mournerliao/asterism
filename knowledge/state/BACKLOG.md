...
- [x] **Collection Dial buildable spec 与 tickets（Phase 2.2，2026-08-12，GitHub #29–#34）**：规格已裁决候选冻结与静默降级、单项 / 多选统一持久写入 seam、More / New 承接、ADR 0034 effective mutation receipt Undo、虚拟列表 / Quick Look / 移动误触协调、原型退役和真实连续任务验收。用户批准压缩为 5 张 tracer-bullet tickets，GitHub 原生线性依赖为 #30 → #31 → #32 → #33 → #34；当前唯一可领取 frontier 为 #30。尚未进入实现。见 `logs/2026-08-12-collection-dial-spec.md` 与 `logs/2026-08-12-collection-dial-tickets.md`。
