...
- [x] **Collection Dial buildable spec 与 tickets（Phase 2.2，更新于 2026-08-12，GitHub #29–#34）**：规格已裁决候选冻结与静默降级、单项 / 多选统一持久写入 seam、More / New 承接、ADR 0034 effective mutation receipt Undo、虚拟列表 / Quick Look / 移动误触协调、原型退役和真实连续任务验收。用户批准压缩为 5 张 tracer-bullet tickets，GitHub 原生线性依赖为 #30 → #31 → #32 → #33 → #34；#30 受信 relation mutation seam 已实现，下一可领取 frontier 为 #31 单项生产 Collection Dial。#30 migration / Edge Function 尚未推送到远端，真实环境 smoke 步骤见 `supabase/README.md`。见 `logs/2026-08-12-collection-dial-spec.md`、`logs/2026-08-12-collection-dial-tickets.md` 与 `logs/2026-08-12-issue-30-trusted-collection-relation-mutations.md`。
