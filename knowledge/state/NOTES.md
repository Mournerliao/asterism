...
- **#21 星图 prototype 两未知数已收口（2026-07-24）**：dev-only 一次性原型 `/dev/star-map-prototype`（`apps/web/src/prototype/star-map/*`，零新增依赖、`import.meta.env.DEV` 门控、浏览器实测）。**Q1 确定性降维**：手搓 PCA（top-2 幂迭代，固定 init 无 RNG）与 PCA-init 确定性精修重投影 **Δmax=0.00e+0（stable✓）**；反面随机力导向 **Δmax=9.24e-1（scrambles✗，换 seed 即全乱）** → #21 定 **PCA top-2 为默认确定性投影、禁随机力导向**（ADR 0026 §7 法则二）。**Q2 分层渲染**：**Canvas2D 足够、免 WebGL**；阈值 `INDIVIDUAL_CAP=900`，>900 走密度场（`GRID=72`，O(cells)）、≤900 走个体；实测 N=5000 density **FPS 117 / visible 4649**、N=800 individual **FPS 66**；投影 PCA@5000≈1.0s（一次性可缓存），精修 / 力导向因 O(N²·D) 精确 kNN 限 N（原型 cap 2000）。**真实坑**：PCA + 裸 min/max 归一化被离群点主导、点云挤角 → 实现须用稳健归一化（分位裁剪 / 按 std 缩放）。截图 `.scratch/retrieval-first/star-map-0{1,2}-*.png`；结论已带回票据草稿 04 与 `logs/2026-07-24-issue21-star-map-prototype-spike.md`。原型代码保留到 phase2 全部实现后再统一清理（用户决策 2026-07-24，dev-only 已隔离，留作实现活参照）。四道门禁全绿（lint/typecheck/test 170/build）。
