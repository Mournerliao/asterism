...
**阶段：Phase 0 脚手架（Scaffold）— 代码侧基本完成；待用户在 Supabase 后台应用迁移并配置 GitHub OAuth 后即可端到端验证登录。**
...
**下一步（需用户在 Supabase 后台操作，无需改码）**：

1. 应用 `supabase/migrations/`（SQL Editor 粘贴或 `supabase db push`）。
2. 配置 GitHub OAuth（GitHub OAuth App + Supabase GitHub provider + Redirect URLs 加 `http://localhost:5173`）。
3. 本地 `pnpm --filter @asterism/web dev` 验证「使用 GitHub 登录」回流并显示当前用户。
...
- [x] 落地初始 schema 与 RLS 迁移（对齐 `contracts/data-model.md`）—— 代码就位；待用户后台应用
- [x] 接通 GitHub OAuth 登录代码（`packages/db` 辅助 + `apps/web` 登录/登出 UI）—— 待用户后台配置 provider 后端到端可用
