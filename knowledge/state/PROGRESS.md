...
> **下一步（恢复点，2026-07-24）**：ADR 0026 检索优先地基与检索交互第一屏已完成到 **#20 隐形混合搜索**。**#21 石墨语义星图**（列表并列可切换的第二视图，非背景层）的两个技术未知数已用 `/prototype` 收口——PCA top-2 确定性投影可复现（Δmax=0.00e+0）、Canvas2D 分层免 WebGL（实测见 `logs/2026-07-24-issue21-star-map-prototype-spike.md` 与 `NOTES.md`），**可直接进入实现**（注意用稳健归一化替代裸 min/max）；dev-only 原型代码保留至 phase2 全部实现后再清理。**#22 涌现簇 + promotion** 仍依赖 #21。每票之间清空 context；动手前先读 ADR 0026 与 `contracts/{product,ui-ux,data-model,architecture}.md`。
