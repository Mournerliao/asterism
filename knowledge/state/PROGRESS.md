...
> **Collection Dial 拟真实体原型已重构（2026-08-10）**：用户否决原有浅弧卡槽 / Dock / 抽象轮盘对比，明确要求接近实体文件夹与半圆容器的拟真空间反馈。开发态 Browse 原型仍位于 `/?prototype=collection-dial`，现已收敛为单一半圆集合盘：仓库拖动超过阈值后带起实体浮层，集合文件夹沿弧线出现；指针靠近目标时文件夹抬升、前片翻开、纸张露出，投放时仓库缩小吸入目标，失败 / retry / Undo 与 Q/E 边界保留。集合盘按实际容器宽度显示 7 / 5 / 3 个奇数窗口，Q/E 越过边缘时窗口跟随目标滑动。已在真实浏览器完成 1536px、1024px、390px、明暗主题与模拟投放验收；尚未完成用户连续操作 verdict，通过前不更新正式产品契约或创建 ADR。见 `logs/2026-08-10-collection-dial-skeuomorphic-prototype.md`。
