...
> **单项生产 Collection Dial #31 已实现（2026-08-13）**：Browse grid / list 均加入独立 44px Grip；Space 或跨过 7px 指针阈值后冻结单仓库 ID 与最多 7 个集合候选，按会话 MRU、更新时间、规范化名称与 ID 稳定排序，并排除已包含该仓库的集合。共享受控 Dial 支持 7 / 5 / 3 奇数窗口、active 居中、Q/E 非循环选择、Enter / 明确按钮确认、目标上 pointerup 直接确认，以及 Escape / 盘外松手 / 卸载取消；隐藏目标退出指针、Tab 与辅助技术。Quick Look 脏笔记会拒绝拿起并保留草稿 / 焦点。提交复用 #30 的 `collection_dial` 持久 operation、client request 幂等与有界恢复，只有服务端成功且 collection query 收敛后才播报成功，失败保留冻结范围与目标。双语、ARIA live、reduced motion、明暗主题与 320 / 390 / 1024 / 1536、200% 有效宽度已验证；全量 lint、typecheck、181+68+37+27 tests 与 build 通过。未实现 #32 多选、#33 More / New / Undo，也未改名或接入 throwaway prototype。下一 frontier 为 #32。见 `logs/2026-08-13-issue-31-production-collection-dial.md` 与 `logs/2026-08-13-ui-generation.md`。
