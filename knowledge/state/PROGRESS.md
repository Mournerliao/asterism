...
> **下一步（恢复点，2026-07-24）**：ADR 0026 的浏览器 embedding 地基已完成到 **#19**。后续 **#20 隐形混合搜索**与 **#21 石墨语义星图**已具备并行实施条件；建议优先对 #20 跑 `/implement`，#21 先用 `/prototype` 收口确定性降维与分层渲染，再进入实现。**#22 涌现簇 + promotion**仍依赖 #21。每票之间清空 context；动手前先读 ADR 0026 与 `contracts/{product,ui-ux,data-model,architecture}.md`。

> #19 浏览器内 embedding 运行时与全库回填已实现（2026-07-24，GitHub #19）：浏览器 Worker 通过 `@huggingface/transformers` 4.2.0 运行固定版本的 `multilingual-e5-small` q8（384 维），模型与 ONNX WASM 资产在构建期校验后同源自托管，运行期禁用远程模型；WebGPU 优先、失败回退 WASM。首次准备有明确约 100–150 MB 双语 UX，用户同意后缓存并以 16 条分块增量回填；每条 owner-only RLS upsert 成功即形成续跑检查点，sync 新增 / 内容变化 / 模型失配自动进入待嵌集合，全量新鲜路径不创建 Worker。运行时失败只降级既有关键词搜索。真实 Chromium 原型测得 q8 模型文件 118,308,185 bytes：WebGPU 初始化 1.00s / 16 条推理 454ms，WASM 初始化 608ms / 16 条推理 236ms（暖缓存）；冷 WebGPU 首次下载与编译 39.25s。`query:` / `passage:` 前缀、en / zh-CN、Graphite Glass 状态条、进度与 a11y 回归均覆盖；四道门禁全绿。真实应用浏览器视觉复核因本轮本地服务器授权额度被系统拒绝而未执行，原型浏览器实测与生产构建已完成。见 `logs/2026-07-24-issue-19-browser-embedding-runtime.md`。
