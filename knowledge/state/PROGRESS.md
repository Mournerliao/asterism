...
> **下一步（恢复点，2026-07-24）**：ADR 0026 检索优先地基与检索交互第一屏已完成到 **#20 隐形混合搜索**。后续 **#21 石墨语义星图**（列表并列可切换的第二视图，非背景层）建议先用 `/prototype` 收口确定性 2D 投影与分层渲染，再进入实现；**#22 涌现簇 + promotion** 仍依赖 #21。每票之间清空 context；动手前先读 ADR 0026 与 `contracts/{product,ui-ux,data-model,architecture}.md`。
