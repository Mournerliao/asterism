...
> **下一步（恢复点，2026-07-24）**：排期决策已拍板——走 **ADR 0026 检索优先范式**（Phase 3 浏览器扩展顺延，下方「下一阶段为 Phase 3」仅历史参考）。ADR 0026 已用 `to-tickets` 拆成 5 个 tracer-bullet 纵向切片并发布为 GitHub Issues **#18–#22**（均 `ready-for-agent`，已接 GitHub 原生 blocked-by 依赖）：**#18** Embeddings 表与 owner-only 写入地基（**已实现，见下方里程碑**）→ **#19** 浏览器内 embedding 运行时与全库回填（blocked by #18，**现已解锁、为下一步**）→ **#20** 隐形混合搜索、**#21** 石墨语义星图（均 blocked by #19，可并行）→ **#22** 涌现簇 + promotion（blocked by #19、#21）。**下一步：对 #19 跑 `/implement`**（#18 已实现并解锁它），每票之间清空 context。**动手前先读** ADR 0026 与 `contracts/{product,ui-ux,data-model,architecture}.md`；#19 / #21 / #22 建议先用 `/prototype` 收口量化档位 / 降维 / 聚类参数再实现。
