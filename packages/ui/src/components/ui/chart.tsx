...
function ChartStyle({ id, config }: { id: string; config: ChartConfig }) {
  ...
  return (
    <style
      // biome-ignore lint/security/noDangerouslySetInnerHtml: shadcn chart injects CSS variables from config
  ...
  (
    ...
    return (
      <div
        ref={ref}
        className={cn(
          'asterism-glass-overlay grid min-w-32 items-start gap-1.5 rounded-lg border px-2.5 py-1.5 text-xs',
          className,
        )}
      >
...
const ChartLegendContent = React.forwardRef<
  HTMLDivElement,
  React.ComponentProps<'div'> & {
    payload?: Array<{ value?: string; type?: string; dataKey?: string | number; color?: string }>;
